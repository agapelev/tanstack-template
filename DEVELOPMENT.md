...
git push origin main сохраняет полную историю для отката
Гибкость
	
Один репозиторий → можно деплоить куда угодно через GitHub Actions
Безопасность
	
Секреты (.env) не попадают в публичные дашборды хостингов
Философия
	
«Не силой и не крепостью, но Духом» — не гнаться за «быстрым деплоем», а строить надёжно
Наш рабочий процесс:
...
Не нужно писать API-эндпоинты: useQuery/useMutation работают «из коробки»
Типобезопасность
	
Авто-генерация типов из schema.ts
Бесплатный тир
	
Достаточно для разработки и небольших проектов
Наш сценарий
	
Сохранение диалогов чата — идеальное применение
Структура convex/:
...
📄 3. convex/schema.ts (полный файл, для подтверждения)
...
🔹 Способ 2: Фоновый процесс через & (Fish shell)
...
✅ Плюсы: Один терминал
⚠️ Минусы: Логи смешиваются; чтобы остановить convex dev, нужно kill %1 или fg
🔹 Способ 3: concurrently (профессиональный подход)
...
📚 Подробнее: Документация TanStack Store
 | Документация Convex
📚 Узнать больше
Ресурс
	
Описание
📘 Документация TanStack
...
У нас уже есть консоль + терминал + ИИ-ассистенты
	
Используем глобальные обработчики
«Это обязательно для продакшена»
	
Нет. Многие проекты живут без Sentry
	
Откладываем, пока не появятся реальные пользователи
«14 дней пробного периода»
	
Мало для оценки, потом — платно
	
Не тратим время на настройку «на потом»
...
Совместимость с TanStack Router 1.157
"@tanstack/*": "^1.157.16"
	
"@tanstack/*": "1.157.16"
	
Точные версии — гарантия совместимости
(нет)
	
"overrides": { "@babel/traverse": "^7.26.10" }
	
Устранение конфликта buildError
🔹 tsconfig.json
...
Асинхронная отправка в Convex
	
Логирование без блокировки UI
import.meta.env.VITE_... вместо process.env
	
Корректная работа с Vite
...
Исключить node_modules/ из компиляции
(отсутствовало)
	
"isolatedModules": true
	
Критично для Vite + React + код-сплиттинг
(отсутствовало)
...
Откройте http://localhost:3000, отправьте сообщение
История сохраняется
	
Перезагрузите страницу → диалог должен остаться
Ошибки логируются
	
В консоли браузера: throw new Error("Тест") → проверьте Convex Dashboard → Data → errors
Devtools
	
Нажмите иконку в левом нижнем углу → изучите состояние роутера
🔹 Коммит и пуш
...
Решение
buildError: Cannot read properties of undefined
	
1. Зафиксировать версии TanStack в package.json
2. Добавить overrides для Babel
3. Очистить кэш: rm -rf node_modules/.vite .tanstack-router
Convex не подключается
	
1. Убедиться, что bunx convex dev запущен
2. Проверить VITE_CONVEX_URL в .env
3. Перезапустить оба сервера
Ошибки TypeScript после обновления
	
1. Проверить tsconfig.json: isolatedModules: true
2. Очистить кэш: rm -rf node_modules/.vite
3. Перезапустить bun run dev
Sentry не отправляет ошибки
	
1. Проверить VITE_SENTRY_DSN в .env
2. Убедиться, что src/sentry.ts импортируется в client.tsx
3. Протестировать: throw new Error("Тест")
Devtools панель мешает
	
1. Нажать иконку «глаз» для скрытия
2. Или обернуть <TanStackRouterDevtools /> в import.meta.env.DEV && ...
...
Приоритет
🗣️ Русский системный промпт
	
Настроить в настройках чата (шестерёнка): язык, стиль, духовное измерение
	
🔹 Высокий
🔗 Интеграция с ~/web-dev/ai
	
Подключить чат как виджет к основному проекту
	
🔹 Высокий
🌐 Деплой через GitHub Actions
	
Настроить .github/workflows/deploy.yml для автоматического деплоя
	
🔸 Средний
🧠 Улучшение схемы Convex
	
Добавить поля: model, tokens, tags для аналитики
	
🔸 Средний
🎨 Кастомизация UI
	
Темы, шрифты, анимации — под стиль «Цитадель духа»
	
🔹 Низкий
📊 Дашборд администратора
	
Страница /admin для просмотра ошибок, статистики, управления
...
Не добавлять инструмент, пока не поняли, зачем он нужен
Контроль прежде удобства
	
git push вместо «быстрого деплоя» — ради истории и отката
Служение прежде функциональности
	
Код не просто работает — он назидает, учит, вдохновляет
Память прежде забвения
	
Документировать каждый шаг — чтобы будущий «я» не начинал с нуля
Дух прежде буквы
	
Не гнаться за «модными» технологиями, а выбирать то, что служит цели
...
```fish
# 1. Создайте/замените файлы в корне проекта:
nano README.md          # вставьте первый блок
nano DEVELOPMENT.md     # вставьте второй блок
...
# 3. (Опционально) Проверьте рендеринг на GitHub:
#    Откройте репозиторий в браузере → убедитесь, что Markdown отображается корректно

    🤝 Лев Николаевич, теперь у вас есть два документа-памятника. 🙏
    README.md — для тех, кто придёт после.
    DEVELOPMENT.md — для вас, когда вернётесь.  
    Не сила, не спешка — но последовательность и Дух.
    Не забыть — но помнить. Не начать с нуля — но продолжить с того места, где остановились.
...
```tsx
// В компоненте:
const { data: conversations } = useQuery(api.conversations.list);
